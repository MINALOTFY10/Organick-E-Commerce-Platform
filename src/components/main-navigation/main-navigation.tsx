    ...
    >
      <div className="mx-auto flex w-full max-w-350 items-center px-4 md:px-8 lg:px-16">
        <div className="flex gap-8 md:gap-16 flex-1 items-center">
          <motion.div animate={{ scale: isScrolled ? 0.92 : 1 }} transition={{ duration: 0.3 }}>
            <BrandLogo />
          </motion.div>

          <div className="ms-auto lg:ms-0 me-2 hidden max-sm:block">
            <NavSearch onNavigate={() => setMenuOpen(false)} />
          </div>
          <NavLinks menuOpen={menuOpen} setMenuOpen={setMenuOpen} session={session} cartCount={cartCount} />
        </div>
