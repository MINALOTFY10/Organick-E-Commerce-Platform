...
import { User, Mail, Lock, ArrowRight, Loader2, Sprout, Github } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect to the email verification page so the user can enter their code.
      // The email param lets the verify page pre-fill the address field.
  ...
  const handleSocialLogin = async (provider: "google" | "github") => {
    ...
    if (error) {
      setError(error.message || `Failed to continue with ${provider}`);
    ...
    <div className="min-h-screen flex w-full bg-[#F9F8F4]">
      ...
      <div className="hidden lg:flex w-1/2 relative bg-(--primary-color) items-center justify-center overflow-hidden">
        ...
        <div
          ...
          style={{
            backgroundImage: "url('https://images.unsplash.com/photo-1500937386664-56d1dfef3854?q=80&w=2070&auto=format&fit=crop')", // Wheat field / Farm image
        ...
        >
          <div className="text-center p-8">
            <Sprout className="w-16 h-16 text-(--secondary-color) mx-auto mb-4" />
            <h1 className="text-4xl font-serif text-white mb-2">Start Fresh</h1>
            <p className="text-white/70 text-lg">Join the eco-friendly movement today.</p>
          </div>
        </motion.div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 lg:p-16 relative">
        <motion.div
          initial={{ opacity: 0, x: -20 }} // Comes from left this time
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.6 }}
          className="max-w-md w-full space-y-8 bg-white p-10 rounded-3xl shadow-xl border border-gray-100"
        >
          <div className="text-center">
            <h2 className="text-3xl font-bold text-(--primary-color) font-serif">Create Account</h2>
            <p className="mt-2 text-sm text-gray-500">Enter your details to get started with Organick.</p>
          </div>
          ...
          <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
            ...
            <div className="space-y-4">
              ...
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <User className="h-5 w-5 text-gray-400 group-focus-within:text-(--secondary-color) transition-colors" />
                </div>
                <input
                  id="name"
                  name="name"
            ...
            <motion.button
              ...
              whileHover={{ scale: 1.01 }}
              whileTap={{ scale: 0.98 }}
              className="group relative w-full flex justify-center py-3.5 px-4 border border-transparent text-sm font-bold rounded-xl text-white bg-linear-to-r from-(--primary-color) to-[#387086] hover:from-[#1f3a4a] hover:to-[#2c5a6f] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-(--secondary-color) disabled:opacity-70 disabled:cursor-not-allowed shadow-lg transition-all duration-200 mt-2 cursor-pointer"
            >
              {loading ? (
            ...
            <div className="text-center mt-4">
              ...
              <a href="/login" className="font-bold text-(--primary-color) hover:text-(--primary-color)/80 transition-colors text-sm">
                Sign in here
              </a>
            </div>
          </form>
